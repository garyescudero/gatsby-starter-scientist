...
//     }
    ...
    const gsPublicationsByYear = gsQuery.googleScholarPublications && gsQuery.googleScholarPublications.list.length > 0
      ? sortArticles( gsQuery.googleScholarPublications.list)
      : null;

      
  return (
    gsPublicationsByYear
    && <Publications publications={gsPublicationsByYear} />
  );
};
